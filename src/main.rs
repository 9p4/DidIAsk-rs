...
impl EventHandler for Handler {
    fn message(&self, ctx: Context, msg: Message) {
        ...
        let number = rand::thread_rng().gen_range(
            0,
            env::var("CHANCE")
                .expect("Expected a chance in ENV")
                .parse::<usize>()
                .unwrap(),
        );
        ...
        match global.get(&GlobalKeys::StatusPhrase) {
            Some(value) => {
                ...
                match (status_prefix, phrase) {
                    ...
                    (ActivityType::Listening, _) => ctx.set_activity(Activity::listening(phrase)),
                    (_, _) => {
...
#[group]
#[commands(
    owo, uwu, smile, hug, flex, animal, surprise, dance, shrug, flip, unflip, sus, cri, yike, bear,
    fight
)]
struct Emoji;
...
fn main() {
...

    {
        ...
        let global = data
            .get_mut::<GlobalInformation>()
            .expect("Expected GlobalInformation Hashmap");

        let channel_whitelist = match env::var("CHANNEL_WHITELIST") {
            Ok(val) => Some(val),
            Err(e) => {
